let xhrHook = {
  init: cb => {
    ...
    xhr.prototype.send = function(value) {
      let _self = this;
      this._eagle_start_time = Date.now(); //记录开始请求时间
      ...
      let ajaxEnd = event => () => {
        if (_self.response) {
          ...
          _self._eagle_xhr_info.success =
            ...
            _self.status === 304;
          _self._eagle_xhr_info.duration = Date.now() - _self._eagle_start_time; //关键指标，请求响应时间
          ...
          _self._eagle_xhr_info.param =
            typeof value === 'object' ? JSON.stringify(value) : value;
          _self._eagle_xhr_info.response = _self.response;
    ...
    if (window.fetch) {
      ...
      window.fetch = function() {
        ...
        return _origin_fetch.apply(this, args).then(function(response) {
          ...
          fetchData.duration = Date.now() - startTime; //关键指标，总体的请求时间
          cb(fetchData);
          return response;
        });
      };
